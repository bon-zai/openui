...
flowchart TD
    FS["FullScreen"]
    PM["processMessage()"]
    MF["messageFormat.toApi() — openAIMessageFormat"]
    RH["/api/chat — Route Handler"]
    OAI["OpenAI — chat.completions.create() — stream: true"]
    RS["response.toReadableStream() — NDJSON"]
    SP["openAIReadableStreamAdapter() — parses stream"]
    AM["Rendered assistant message (text or Generative UI)"]

    FS -->|"user message"| PM
    PM --> MF
    MF -->|"POST with OpenAI messages + systemPrompt"| RH
    RH --> OAI
    OAI --> RS
    RS --> SP
    SP --> AM
